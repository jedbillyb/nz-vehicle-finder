...
import { tlaToSlug, slugToTla, titleCaseRegion } from "@/lib/slugs";
...
export default function RegionStats() {
  ...
  useEffect(() => {
    ...
    const description =
      total !== null
        ? `${totalText} vehicles are registered in ${tlaDisplay}, New Zealand.${top.make ? ` Most popular make is ${top.make.value.charAt(0) + top.make.value.slice(1).toLowerCase()}.` : ""} View full breakdowns from the Motor Vehicle Register.`
    ...
    >
      <div style={{ flex: 1 }}>
        <header style={{ borderBottom: "1px solid #e5e7eb", background: "#ffffff" }}>
          ...
          <div className="header-main" style={{ padding: "10px 24px", display: "flex", alignItems: "center", gap: 16, background: "#ffffff" }}>
            <Link to="/" style={{ textDecoration: "none" }} onClick={() => captureEvent("logo_home_clicked", { source: "region_stats_page", tla: tlaUpper })}>
      ...
      <footer className="footer-root" style={{ padding: "12px 24px", background: "#ffffff", borderTop: "1px solid #e5e7eb", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, fontSize: 10, fontFamily: "'JetBrains Mono', 'Courier New', monospace", color: "#6b7280", letterSpacing: "0.1em" }}>
        <div className="footer-links" style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <a href="https://github.com/jedbillyb/nz-vehicle-finder" target="_blank" rel="noopener noreferrer" onClick={() => captureEvent("github_link_clicked", { location: "footer", source: "region_stats_page", tla: tlaUpper })} style={{ color: "#111827", textDecoration: "none", display: "flex", alignItems: "center", gap: 4 }}>
